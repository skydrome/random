...
# and optionally into a Mozilla prefs file.
#
# Examples:
#
...
use constant {
   ...
   BROWSER_VERSION                   => "7.0.2.3",
   BROWSER_REVISION                  => '1.9.1.2',
   BROWSER_RELEASE_DATE              => '20090610',
};
...
sub log_to_file ($) {
    ...
    printf LOGFILE " ($logtime) $message\n";
    close(LOGFILE);
...
[B<--quiet>] [B<--sleeping-time> I<minutes>] [B<--silent>]
