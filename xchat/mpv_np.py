    ...
    try:
        with MpvIpcClient.for_platform() as mpv:
            ...
            if fileformat in audioformats:
                ...
                CMD_FMT_AUDIO = (
                    "me is listening to: \x0311\x02${metadata/artist}\x0F - \x0311\x02${media-title}\x0F ("
                    + size
                    + " • ${audio-bitrate} • ${audio-params/channel-count}ch) • [${time-pos}${!duration==0: / ${duration}}]"
                )
                ...
                CMD_FMT_STREAM = (
                    "me is streaming: \x0311\x02${media-title}\x0F - \x0311\x02"
                    + uploader
                    + "\x0F • [${time-pos}${!duration==0: / ${duration}}]"
                )
                ...
                size = prettiBytesSize(mpv.command("get_property", "file-size"))
                CMD_FMT_VIDEO = (
                    "me is watching: \x0311\x02${filename}\x0F • "
                    + size
                    + " • [${time-pos}${!duration==0: / ${duration}}]"
                )
...
if __name__ == "__main__":
    help_str = (
        ...
        "(or adjust the path in the script source).".format(
            path=WIN_PIPE_PATH if sys.platform == "win32" else UNIX_PIPE_PATH
        )
    )
